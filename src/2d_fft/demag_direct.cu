 ...
 *
 * Note: OVERWRITES h_out_dev (matches FFT version's new contract).
 */
void Demag_Apply(DemagData *d, const double *y_dev, double *h_out_dev)
{
    ...
    /* scatter h_x/h_y/h_z into SoA h_out_dev, scaled by strength.
     * OVERWRITES h_out_dev (main zeroes it before calling us, so
     * overwrite vs accumulate gives the same result; overwrite is
     * cleaner and matches the FFT version's contract). */
    ...
    cudaMemcpy(h_out_dev, h_hout, (size_t)3*ncell*sizeof(double), cudaMemcpyHostToDevice);
