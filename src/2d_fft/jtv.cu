/*
 * jtv.cu — Analytic Jv for exchange + z-axis anisotropy + DMI.
 *
 * Demag contribution to Jv is NOT included: adding it would require a
 * second FFT pipeline per Jv call, and GMRES + the preconditioner
 * tolerate the inexactness.  The analytic piece covers all the local
 * stencil terms (linear in y) and the dominant self-coupling through
 * anisotropy.
 *
 * JtvUserData mirrors UserData's first 3 pointers + the ints:
 *   offset 0  : void* pd_opaque
 *   offset 8  : void* demag_opaque
 *   offset 16 : void* d_hdmag_opaque
 *   offset 24 : int nx, ny, ng, ncell, neq
 *   offset 48 : double nxx0, nyy0, nzz0   (not used here)
 */
    ...
    int ng, int ny, int ncell)
{
  ...
  /* DMI is along x (c_nsk={1,0,0}): L/R neighbors of mx get coeff (che+chb).
   * Anisotropy is along z (c_msk={0,0,1}): only h3 picks up chk*m3. */
  ...
  /* ∂h/∂v: DMI contributes only to dh1; anisotropy only to dh3. */
  const sunrealtype dh1 = che_chb*(v1L+v1R)+jc_che*(v1U+v1D);
  const sunrealtype dh2 = jc_che*(v2L+v2R+v2U+v2D);
  const sunrealtype dh3 = jc_che*(v3L+v3R+v3U+v3D) + jc_chk*v3;
