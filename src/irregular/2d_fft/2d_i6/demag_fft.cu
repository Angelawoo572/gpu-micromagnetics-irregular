 ...
 *
 * UNCHANGED from i5: the FFT demag pipeline operates on the full grid
 * regardless of geometry.  Hole cells contribute 0 to the input FFT
 * (since y[hole]=0), so they don't contaminate active cells' output.
 * Hole entries of h_out are garbage but unused (RHS only reads h_dmag
 * at active cells).
 *
