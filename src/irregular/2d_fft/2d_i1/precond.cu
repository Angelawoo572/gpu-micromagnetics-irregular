 ...
 *
 * Only nonzero self-derivative: dH1/dm1 = c_chk (x-axis anisotropy).
 *
 * d_i = d(m.H)/dm_i (neighbors frozen):
 *   d1 = h1 + m1*c_chk
 *   d2 = h2
 *   d3 = h3
 *
 * KEY FIX: column 0 gains three c_chk cross-terms that were missed before:
    ...
    int ng, int ny, int ncell)
{
  ...
  /* Row 1 */
  d_J[b+3] = pc_chg*(h3 - pc_chk*m3) - pc_alpha*d1*m2;
  d_J[b+4] = pc_alpha*(-d2*m2 - mh);
  d_J[b+5] = -pc_chg*h1 - pc_alpha*d3*m2;

  /* Row 2 */
  d_J[b+6] = pc_chg*(pc_chk*m2 - h2) - pc_alpha*d1*m3;
