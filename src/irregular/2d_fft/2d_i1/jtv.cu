/*
 * jtv.cu — Analytic Jv for exchange + x-axis anisotropy ONLY.
 *
 * When demag is on, this function is NOT registered; CVODE uses its own
 * finite-difference Jv approximation that calls f(), automatically
 * including demag. This avoids inconsistencies between analytic Jv
 * and f() that caused Newton to stall mid-trajectory.
 *
 * JtvUserData is 3-pointer matching UserData's first 3 pointers:
